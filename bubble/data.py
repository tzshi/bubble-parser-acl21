...
class DataProcessor(Dataset):
    def __init__(self, filename, parser, modules, train=False):
        ...
        if train:
            self.data = [x for x, y in zip(self.data, data) if y.in_train]
...
class DataCollate:
    ...
    def __call__(self, data):
        ...
        for idx, (seq, xpos, seqlen) in enumerate(
            zip(words, xposs, word_seq_lengths)
        ):
            seqlen = seqlen.item()
